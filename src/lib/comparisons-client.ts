/**
 * Client-side DeepSeek comparison engine.
 * Calls DeepSeek API directly from the browser to avoid Vercel function timeouts.
 * Result is then saved to the server for history persistence.
 */
...
/**
 * Analyze bookmarks by calling DeepSeek directly from the browser.
 * Uses fetch with a generous timeout — the browser has no 60s cap.
 */
export async function analyzeComparisonFromBrowser(
  apiKey: string,
  bookmarks: {
    id: string;
    title: string;
...
4. recommendedOrder 是建议阅读顺序的 bookmarkId 数组`;
  ...
  const timeoutId = setTimeout(() => controller.abort(), 120_000); // 2 minute browser-side timeout
  ...
  try {
    ...
    if (data.usage) {
      console.log(`[DeepSeek comparison] Tokens: ${data.usage.total_tokens} (in: ${data.usage.prompt_tokens}, out: ${data.usage.completion_tokens})`);
    }
