...
export function BookmarkCard({ bookmark, onStatusChange, onDelete, onEdit }: BookmarkCardProps) {
  ...
  return (
    <ContextMenu>
      <ContextMenuTrigger>
        <Card className="overflow-hidden transition-all duration-300 hover:shadow-[0_8px_40px_rgba(0,0,0,0.06)] rounded-2xl border border-[#2c2c2c]/[0.04] bg-white/60 backdrop-blur-sm group/card hover:-translate-y-0.5">
          {/* Cover Image */}
          {bookmark.coverImage && (
            <a
              href={bookmark.url}
              ...
              className="block relative aspect-[2.2/1] bg-[#f5f3f0] overflow-hidden"
            ...
            >
              ...
              {bookmark.contentType === "video" && (
                <div className="absolute bottom-2.5 left-2.5 bg-[#2c2c2c]/70 backdrop-blur-md text-white/90 text-[10px] px-2.5 py-1 rounded-full font-medium flex items-center gap-1.5">
          ...
          <div className="p-4">
            ...
            <div className="flex items-start gap-2 mb-2.5">
              ...
              <div className="flex items-center gap-1.5 min-w-0 flex-1">
                ...
                <span className="text-[11px] text-[#2c2c2c]/30 truncate font-sans">
              ...
              <button
                ...
                className="shrink-0 flex items-center gap-1 text-[10px] text-[#2c2c2c]/35 hover:text-[#2c2c2c]/60 transition-colors cursor-pointer group/status"
                data-no-drag
              >
            ...
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="font-semibold text-[14px] leading-snug line-clamp-2 text-[#2c2c2c]/85 hover:text-[#b76e4b] transition-colors inline-flex items-start gap-1 font-sans"
              onClick={(e) => e.stopPropagation()}
            >
              {bookmark.title}
              <ExternalLink className="h-3 w-3 shrink-0 mt-0.5 text-[#2c2c2c]/10 opacity-0 group-hover/card:opacity-100 transition-opacity" />
            </a>

            {/* AI summary */}
            {!!bookmark.aiSummary && (
              <p className="text-[12px] text-[#2c2c2c]/35 mt-1.5 line-clamp-2 leading-relaxed font-sans">
            ...
            {bookmark.contentType === "repository" && (
              <div className="flex items-center gap-3 mt-2 text-[11px] text-[#2c2c2c]/35 font-sans">
                {meta.language && (
                  <span className="flex items-center gap-1.5">
                    <span className="h-2 w-2 rounded-full bg-[#b76e4b]" />
            ...
            {bookmark.contentType === "article" && meta.author && (
              <div className="flex items-center gap-1 mt-2 text-[11px] text-[#2c2c2c]/25 font-sans">
            ...
            {bookmark.tags.length > 0 && (
              <div className="flex flex-wrap gap-1.5 mt-3">
                {bookmark.tags.map(({ tag }) => (
                  <Badge key={tag.id} variant="secondary" className="text-[10px] px-2 py-0.5 rounded-lg bg-[#f5f3f0] text-[#2c2c2c]/40 border-0 font-normal font-sans">
                    {tag.name}
                  </Badge>
                ))}
              </div>
            ...
            {/* Timestamp */}
            <p className="text-[10px] text-[#2c2c2c]/20 mt-2.5 font-sans">
      ...
      <ContextMenuContent className="w-52 rounded-2xl border border-[#2c2c2c]/[0.04] shadow-[0_12px_40px_rgba(0,0,0,0.08)] bg-white/95 backdrop-blur-xl p-1.5">
        <ContextMenuSub>
          <ContextMenuSubTrigger className="text-[13px] rounded-xl font-sans">
            更改状态
          </ContextMenuSubTrigger>
          <ContextMenuSubContent className="w-44 rounded-2xl border border-[#2c2c2c]/[0.04] bg-white/95 backdrop-blur-xl p-1.5">
            {Object.entries(STATUS_CONFIG).map(([key, conf]) => {
              ...
              return (
                <ContextMenuItem
                  ...
                  className={cn("text-[13px] rounded-xl font-sans", bookmark.status === key && "font-medium bg-[#f5f3f0]")}
                >
                  <Icon className="h-3.5 w-3.5 mr-2.5 opacity-50" />
                  {conf.label}
                  {bookmark.status === key && (
                    <CheckCircle2 className="h-3.5 w-3.5 ml-auto text-[#b76e4b]" />
                  )}
                </ContextMenuItem>
              );
            })}
        ...
        <ContextMenuSeparator className="bg-[#2c2c2c]/[0.03]" />
        <ContextMenuItem
